...
def write_single_slide_time_evo(out_file,cube_name,layer,timesteps,postfix):
    ...
    for timestep in timesteps:
        ...
        out_file.write(f"\\includegraphics[width=0.32\\textwidth]{{{cube_name}-level{layer}-{timestep:04d}{postfix}.png}} {end_line}\n")
    end_slide(out_file)
...
#    timesteps = [0,20,50,100,200,500]

    postfix = "_steps_before_crash"
#    postfix = "_steps_from_zero"
    ...
    with open("map_presentation.tex","w") as f:
        ...
        for cube in cubes:
            for layer in cubes[cube]:
                write_single_slide_time_evo(f,cube,layer,timesteps,postfix)
        close_slides(f)
